...
const LOCAL_FALLBACK_PROBLEMS: Problem[] = [
  ...
  },
  {
    ...
    description: "### Goal\nConstruct a declarative Antigravity Skill documentation (`SKILL.md`) for a custom log-parsing service.\n\n### Starter State\n`SKILL.md` contains an unconfigured starter stub, and the implementation script `parse.py` is pre-solved and read-only to keep focus strictly on the declarative skill configuration in markdown.\n\n### Backstory\nAntigravity agents need the capability to analyze system event logs without leaving their agent sandbox. This skill accepts log lines, applies pattern heuristics, and outputs structured analytical breakdowns.\n\n### Task\n1. Author detailed instructions and YAML frontmatter headers in `skills/log_parser/SKILL.md`.\n2. Do NOT try to modify `skills/log_parser/scripts/parse.py` (it is pre-solved and read-only for security reasons).\n3. Keep the markdown clean and standard, declaring name, description, and key specifications.\n\n### Verification\nYour custom skill documentation will be loaded by a test harness and verified for formatting compliance, correct frontmatter keywords, and alignment with the parsing suite.",
    difficulty: "hard",
  ...
  },
  {
    ...
    description: "### Goal\nConstruct an Antigravity Skill (`k8s_triage`) that inspects Pod crash loops and decodes container config states safely.\n\n### Starter State\n`SKILL.md` contains an unconfigured starter stub, and the implementation script `triage.py` is pre-solved and read-only to keep focus strictly on the declarative skill configuration in markdown.\n\n### Backstory\nOn-call engineers are inundated with high-dimensional K8s cluster alerts. We need a specialized declarative skill that queries crash telemetry logs and filters noise within strict security limits.\n\n### Task\n1. Author detailed instructions and YAML frontmatter headers in `skills/k8s_triage/SKILL.md`.\n2. Do NOT try to modify `skills/k8s_triage/scripts/triage.py` (it is pre-solved and read-only for security reasons).\n3. Keep the markdown clean and standard, declaring name, description, and key specifications.\n\n### Verification\nYour custom skill documentation will be loaded by a test harness and verified for formatting compliance, correct frontmatter keywords, and alignment with the triage suite.",
    difficulty: "medium",
    category: "skill_verification",
  ...
  },
  {
    ...
    description: "### Goal\nCreate an Antigravity Skill (`schema_migrator`) that validates index safety and generates safe transaction rollback scripts.\n\n### Starter State\n`SKILL.md` contains an unconfigured starter stub, and the implementation script `migrate.py` is pre-solved and read-only to keep focus strictly on the declarative skill configuration in markdown.\n\n### Backstory\nDatabase migrations frequently trigger long-lived table locks, blocking API traffic. We need a secure skill to audit DDL index plans before execution.\n\n### Task\n1. Author detailed instructions and YAML frontmatter headers in `skills/schema_migrator/SKILL.md`.\n2. Do NOT try to modify `skills/schema_migrator/scripts/migrate.py` (it is pre-solved and read-only for security reasons).\n3. Keep the markdown clean and standard, declaring name, description, and key specifications.\n\n### Verification\nYour custom skill documentation will be loaded by a test harness and verified for formatting compliance, correct frontmatter keywords, and alignment with the migrator suite.",
    difficulty: "medium",
    category: "skill_verification",
